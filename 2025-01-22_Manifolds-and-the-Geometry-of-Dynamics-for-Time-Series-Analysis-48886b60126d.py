...
def false_nearest_neighbors(series, delay, max_dim, threshold=10):
    ...
    for dim in range(1, max_dim + 1):
        ...
        distances = (
            pairwise_distances(embedded[:-1]) + 1e-10
...
correlation = np.corrcoef(
    source_series[dimension : dimension + min_length], predicted_source[:min_length]
)[0, 1]
...
bond_yield = np.sin(t - np.pi / 6) + 0.05 * np.random.randn(
    len(t)
)  # Bond yield, lagged by phase shift
